...
La configuración de contenedores queda separada para desarrollo local: `compose.yaml` exige `MBAPO_AUTH_SECRET` y `POSTGRES_PASSWORD`, publica PostgreSQL únicamente en localhost y deja el bind de la aplicación explícito. El healthcheck de `Dockerfile` consulta `/api/health` usando el `PORT` efectivo y da margen a las migraciones; `.env.example`, `docs/docker.md` y la lista de producción documentan secretos, staging, rollback y eliminación segura de volúmenes. `test/deployment-config.test.js` protege estas reglas sin requerir Docker. La construcción y ejecución real de la imagen siguen cubiertas por CI/staging porque Docker no está instalado en la estación local.

El PWA usa `mbapo-static-v4`, un caché versionado limitado al shell y a activos estáticos del mismo origen. Las navegaciones son network-first y, sin red, muestran `public/offline.html`; el worker no intercepta ni almacena `/api`, peticiones autenticadas o idempotentes, ni respuestas privadas/no-store. `test/pwa-offline.test.js` verifica el manifiesto, los activos declarados, el aislamiento de sesiones/API y el fallback de navegación.
